...
private var prePosition = null;
private var isLockCamera = false;
...
//	DebugDrawStuff();
	...
	updateVector = currentRotation * Vector3.back * distance;
	cameraTransform.position += updateVector;
	...
	if (prePosition != null && cameraTransform.position.z < prePosition.z && prePosition.z - cameraTransform.position.z < 100) {
		cameraTransform.position.z = prePosition.z;
	}
	if (prePosition != null && cameraTransform.position.x < prePosition.x) {
		cameraTransform.position.x = prePosition.x;
	}
	if (!isLockCamera) {
...
function SetLockCamera(b : boolean) {
	isLockCamera = b;
}

function LockCamera() {
	this.SetLockCamera(true);
}
